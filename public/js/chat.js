...
function generate_post(id) {
    ...
    post.find(".chat_label")
        .click(function() {
            set_channel(chat[id].board, chat[id].post_id);
            return false;
        });
...
function update_chat(new_data, first_load) {
    "use strict";
    //console.log(`UPDATE_CHAT (${first_load}):\n${JSON.stringify(new_data)}`);
    // Check if this post number already exists
    var id = new_data.post_id;
    ...
    if (changed.board && chat_id === "all") {
        post.find(".chat_label")
            .css("display", "inline")
            .attr("href", "/chat/" + data.board + "#" + id)
            .text("/" + data.board);
    }
    ...
    if (changed.chat || changed.convo_id) {
        var is_op = data.original_poster;
        if (data.chat !== "" && data.chat !== "General" &&
            $.inArray(data.chat, Object.keys(convo_map)) < 0 &&
            is_op){
            convo_map[data.chat] = data.post_id;
        }
        post.toggleClass("convo_op", is_op);
        var chat_convo = post.find(".chat_convo");
        chat_convo
		.text(data.chat + (is_op ? " (OP)" : ""))
        .css({
        	background:getRandomColor(hashString(data.chat))
        });
        if (!is_op) chat_convo.data("dest", convo_map[data.chat]);
    }
    if (changed.chat || new_post) {
        apply_filter(post);
    }
    if (changed.date) {
    ...
    if (new_post) {
        ...
        var convo_index = $.inArray(data.chat, convos);
        //console.log(`chat index: ${convo_index}`);
        if (convo_index < 0) {
            convos.push(data.chat);
        ...
        if (!first_load) notifications(data.chat, for_you);
...
/*    if (value === "highlight"){
        ...
        posts.toggleClass(function () {
            var id = parseInt(this.id.match(/\d+/)[0], 10);
            return ($.inArray(chat[id].chat, highlighted_convos) > -1) ? '' : 'chat_hidden';
        }, true);
   // }
}
...
function pull_chats(new_channel, new_post) {
    ...
    $.ajax({
        ...
        //console.log(`Finished /data/${new_channel}: ${JSON.stringify(data_chat)}`);
        draw_data = data_chat.data;
        draw_data.sort(function(a, b) {return b.count - a.count;});
        draw_chat(draw_data);
        $('.chats').toggleClass('shown', true);
        on_chat = function(d) {
            //console.log(`Found chat: ${d}`);
            update_chat(JSON.parse(d));
            if($("#autoscroll").prop('checked')) {
