function apply_rules(data, post, id) {
    ...
    var rules = [
        ...
        [/(?:https?:\/\/)?(?:www\.)?(?:soundcloud\.com)\/(\S+)(?:\?\S+)?/g, function (m, o) {
            var main = $("<span/>");
            var url = "https://soundcloud.com/" + m[1];
            ...
            embed.click(function (e) {
                e.stopPropagation();
                if (embedded) {
                    main.find("div.soundcloud").remove();
                } else {
                  fetch('https://soundcloud.com/oembed?url=' + url).then(async (res) => {
                   main.append('<div class="soundcloud">' + (await res.json()).html + '</div>');
                  })
                }
                embedded = !embedded;
                embed.text(embedded ? "(unembed)" : "(embed)");
                var post = main.parents(".chat");
                post.toggleClass('chat_embed', embedded);// post.find("div.soundcloud").length > 0);
            });
        }],
