...
$(document).ready(function () {
    ...
    socket.on('disconnect', function () {
        console.log("Disconnected from the kot");
...
{
    console.log('Creating server post');
    var data = {};
    data.count = 0;
    data.chat = "";
    data.body = status;
...
function cool_down() {
    ...
    if (cool_down_timer <= 0) {
        console.log(`cool down finished!`);
        ...
        console.log(`cool down: ${cool_down_timer}`);
...
function submit_chat() {
    ...
    if (msg.indexOf('//') !== 0 && msg.indexOf('/') === 0) {
        ...
        data.append("board", $("#board_select").val());
        console.log(`Sending request: POST ${$("#comment-form").attr("action")}`);
        $.ajax({
            xhr: function() {
            ...
            success: (data, status, xhr) => {
                console.log("We're successful!")
                handle_post_response(data)
...
function handle_post_response(resp) {
    ...
    if (!resp.data && resp.message === "session_expiry") {
        $("#body").val(last_post);
        submit_captcha();
    } else if (!resp.data && resp.message === "ban_violation") {
        div_alert("You've been banned.");
        init_cool_down();
    } else if (!resp.data && resp.message === "proxy") {
        div_alert("You are proxy.");
        init_cool_down();
    } else if (!resp.data) {
        div_alert(resp.message);
        init_cool_down();
    } else if (resp.data.post_id && $.inArray(resp.data.post_id, my_ids) < 0) {
        clear_fields();
        init_cool_down();
        my_ids.push(resp.data.post_id);
        if (html5) {
            localStorage.my_ids = JSON.stringify(my_ids);
        }
        if (quote_links_to[resp.data.post_id]) {
            $.each(quote_links_to[resp.data.post_id], function() {
                $(this).text($(this).text() + " (You)");
            });
        }
    } else if (resp.message === "captcha") {
        $("#submit_button").prop("disabled", false);
