        ...
        get_all_settings()
            .then((settings) => {
                settings.forEach(set => {
                    if(set.settings.channels.monoko == "" || set.settings.channels.Monoko == null) return
                    ...
                    options.username = data.name || "Monoko";
                    ...
                    if (data.image) {
                        options.content += `\n\nThis message contains an image from Monoko. [Upgrade to gold to view](<${url}>)`
                        // options.files = [{
